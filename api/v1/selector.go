...
func (in *Selector) FindMatchingResources(ctx context.Context, kubeClient client.Client) ([]unstructured.Unstructured, error) {
	namespaces, err := in.findMatchingNamespaces(ctx, kubeClient)
	if err != nil {
		return nil, err
	}

	// Get matching resources list from all matching namespaces
	rawResources, err := in.findAllMatchingResourcesInNamespaces(ctx, kubeClient, namespaces)
	if err != nil {
		return nil, err
	}
...
func (in *Selector) findMatchingNamespaces(ctx context.Context, kubeClient client.Client) ([]string, error) {
	matchedNamespaces := make([]string, 0)

	// Get all namespaces
	allNamespaces := &v1.NamespaceList{}
	err := kubeClient.List(ctx, allNamespaces)
	if err != nil {
		return nil, err
	}
...
func (in *Selector) findAllMatchingResourcesInNamespaces(ctx context.Context, kubeClient client.Client, namespaces []string) ([]unstructured.Unstructured, error) {
